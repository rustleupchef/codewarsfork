...
"""CodeWars - Check Program Script

Notes:
    * REQUIRES python3 to run
    * If Java, Python3, and/or GCC/G++ are not on your system PATH, then
      you must set the full paths below
    * Data sets are expected to be in a local directory called `student_datasets`
    * Works with .java, .py, .c, and .cpp programs
"""

__version__ = "2025.02.21.1"
...
def get_wide_message(message, fill_char):
    ...
    return f"{fill_char * left_padding} {message} {fill_char * right_padding}"
...
def get_header(message, header_char='-', bar_char='-'):
    ...
    return f"{bar}\n{get_wide_message(message, header_char)}\n{bar}\n"
...
def check(program_file_path, show_input):
    ...
    check_program(problem_number, run_command, show_input)
...
def pre_process_source(problem_number, file_type, filename):
    ...
    if file_type == "java":
        ...
        program_exe_name = f"./prob{problem_number}.exe"
        compile_command = [compiler_path, "-o", program_exe_name, filename, "-lm"]

        if compile_from_source(compile_command, problem_number, filename, file_type):
            # Set the run command if the compile was a success
            run_command = program_exe_name
...
def check_program(problem_number, run_command, show_input_file):
    """
    Check the given program against all data sets
    ...
    for input_file in sorted(os.listdir(STUDENT_DIR)):
        ...
        if result and result.returncode != 0:
            display_error(f"Unable to run your program, the return code was: {result.returncode}")
        ...
        input(f"{'-' * 10} Press ENTER to continue check next data set {'-' * 10}")

    if not found_dataset:
        display_error(f"No dataset files found for problem {problem_number} !")
...
def main():
    print(f"CodeWars Check Program Script v{__version__}\n")
    ...
    if not program_files or len(program_files) != 1:
        display_error(f"Found more than one program in the current directory: {program_files}")
        display_error("Ensure that EXACTLY one program is present in the current directory and try again.")
        return
